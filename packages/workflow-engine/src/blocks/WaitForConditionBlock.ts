...
/**
 * Platform-independent wait-for-condition block handler
 * Uses DOMProvider for DOM operations and provides fallbacks for missing capabilities
 */
export async function handlerWaitForCondition(
  ...
  try {
    ...
    if ((mode === 'manual' || mode === 'auto-or-manual') && hasManualConfirmation) {
      ...
      // Platform-specific UI notification - try to dispatch event if in browser environment
      if (typeof window !== 'undefined' && typeof CustomEvent !== 'undefined') {
        try {
          ...
          console.log('[WaitForCondition] Could not dispatch UI event:', error);
        }
      }
    }
    ...
    const checkAutoConditions = async (): Promise<WaitForConditionResult | null> => {
      // URL 패턴 체크 - 브라우저 환경에서만
      if (conditions.urlPattern && typeof window !== 'undefined' && window.location) {
        try {
      ...
      // 요소 존재 체크 - DOMProvider 사용
      if (conditions.elementExists && domProvider) {
        try {
          const { selector, findBy } = conditions.elementExists;
          const element = await domProvider.findElement({ selector, findBy });
          ...
          console.warn('[WaitForCondition] Element check failed:', error);
        }
      } else if (conditions.elementExists && typeof document !== 'undefined') {
        // Fallback to direct DOM access if no DOMProvider
        try {
          const { selector, findBy } = conditions.elementExists;
          ...
          console.warn('[WaitForCondition] Direct DOM element check failed:', error);
        }
      }

      // 쿠키 존재 체크 - 브라우저 환경에서만
      if (conditions.cookieExists && typeof document !== 'undefined' && document.cookie !== undefined) {
        try {
          ...
          console.warn('[WaitForCondition] Cookie check failed:', error);
        }
      }

      // 스토리지 키 체크 - 브라우저 환경에서만
      if (conditions.storageKey && typeof window !== 'undefined') {
        try {
          ...
          console.warn('[WaitForCondition] Storage check failed:', error);
        }
      }
    ...
    return new Promise<BlockResult<WaitForConditionResult>>((resolve) => {
      ...
      let cleanup = () => {
        ...
        // 표시된 경우 UI 숨김 - 플랫폼 독립적
        if (typeof window !== 'undefined' && typeof CustomEvent !== 'undefined') {
          try {
            window.dispatchEvent(new CustomEvent('8g-hide-execution-status'));
          } catch (error) {
            console.log('[WaitForCondition] Could not dispatch hide UI event:', error);
          }
        }
      };
        ...
        try {
          ...
          cleanup = () => {
            try {
              ...
              console.log('[WaitForCondition] Could not remove event listener:', error);
            }
            originalCleanup();
          };
        } catch (error) {
          console.warn('[WaitForCondition] Could not add event listener:', error);
        }
      }
