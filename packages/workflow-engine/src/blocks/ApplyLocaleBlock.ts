...
export async function handlerApplyLocale(
  ...
  try {
    ...
    const targetLocale = locale || detectLocaleFallback();

    // Check if applyLocale method is available on the DOMProvider
    if (domProvider.applyLocale) {
      // Use DOMProvider's applyLocale method for advanced locale functionality
      await domProvider.applyLocale(targetLocale);
    } else {
      console.warn('[ApplyLocaleBlock] Advanced locale application not supported in this environment, using fallback translation');
    }
    ...
    if (Array.isArray(input)) {
      const translatedArray = input.map((item) => {
        ...
        if (typeof item === 'object' && item !== null && !Array.isArray(item)) {
          return translateObjectFallback(item as Record<string, any>, normalizedMappings, targetLocale);
        }
        // 객체가 아니면 그대로 반환
        return item;
      });
    ...
    if (typeof input === 'object' && input !== null && !Array.isArray(input)) {
      const translated = translateObjectFallback(
...
// Fallback function to detect locale
function detectLocaleFallback(): string {
  // Try to detect locale from various sources
...
// Translate object using locale JSON files
function translateObjectFallback(
  ...
  // Apply translations based on mappings
  for (const [key] of Object.entries(mappings)) {
    if (key in result) {
      const value = result[key];

      if (typeof value === 'string') {
        // Use value directly as translation key
        const translatedValue = getTranslation(value, locale);
        if (translatedValue) {
