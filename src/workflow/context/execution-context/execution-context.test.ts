...
describe('ExecutionContext', () => {
  describe('path resolution (integration)', () => {
    ...
    it('should return undefined for paths without $. prefix', () => {
      ...
      expect(getByPath(context, 'vars.userId')).toBeUndefined();
      expect(getByPath(context, 'forEach.index')).toBeUndefined();
      expect(getByPath(context, 'steps.step1')).toBeUndefined();
    });
