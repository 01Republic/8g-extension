...
import FloatingNotificationButton, { 
  NotificationUrgency 
} from './FloatingNotificationButton';
...
const NotificationManager: React.FC<NotificationManagerProps> = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('8g-update-notification', handleUpdateNotification as EventListener);
    };
  ...
  const handleNotificationClick = (notification: NotificationData) => {
    ...
    chrome.runtime.sendMessage({
      ...
      if (chrome.runtime.lastError) {
        console.error('[NotificationManager] Failed to open side panel:', chrome.runtime.lastError);
        
        // 폴백: Side Panel 열기 실패 시 인라인 UI 표시
        window.dispatchEvent(new CustomEvent('8g-show-check-status-fallback', {
          detail: notification,
        }));
